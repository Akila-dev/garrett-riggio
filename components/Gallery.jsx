...
function Item({ index, position, scale, c = new THREE.Color(), ...props }) {
	...
	// console.log(ref.current.material.scale);
	...
	useFrame((state, delta) => {
		...
		easing.damp3(
			ref.current.scale,
			[clicked === index ? 4.5 : scale[0], clicked === index ? 5 : 4 + y, 1],
			0.15,
			delta
		);
	...
	return (
		<group>
			<Image
				...
				position={position}
				scale={[4.5, 5, 1]}
				// scale={clicked === index ? [4.5, 5, 1] : scale}
				// scale={scale}
				onClick={click}
				onPointerOver={over}
				onPointerOut={out}
...
function Items({ w = 0.7, gap = 0.15 }) {
	...
	return (
		...
		>
			...
			<Scroll>
				{
					urls.map((url, i) => <Item key={i} index={i} position={[i * xW, -0.25, 0]} scale={[w, 4, 1]} url={url} />) /* prettier-ignore */
				}
			</Scroll>
		</ScrollControls>
	);
